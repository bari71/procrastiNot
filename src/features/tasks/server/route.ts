...
const app = new Hono()
    ...
    .post(
        ...
        async (c) => {
            const user = c.get('user');
            const databases = c.get('databases');
            const { name, projectId, status, dueDate, assigneeId, workspaceId } = c.req.valid('json');
    ...
    .patch(
        ...
        async (c) => {
            const user = c.get('user');
            const databases = c.get('databases');
            const { name, projectId, status, dueDate, assigneeId, description } = c.req.valid('json');
            const { taskId } = c.req.param();
